...
export const calculateNutritionTargets = (profile: UserProfile): { 
  ...
  if (profile.dietStrategy === DietStrategy.CarbCycling) {
      if (profile.currentCycleType === CycleDayType.HighCarb) {
          ...
          s.oils = Math.round(s.oils * 1.5 * 10) / 10; 
      }
  }
...
export const analyzeFoodWithGemini = async (
  ...
  return {
    ...
    servings: jsonResult.servings,
    mealType: mealType,
    mainCategory: jsonResult.mainCategory,
...
export const analyzeExerciseWithGemini = async (
  ...
  durationMinutes: number
    ...
    const prompt = `
      ...
      2. Classify as 'Cardio' (Aerobic) or 'Strength' (Anaerobic).
    ...
    return {
        name: jsonResult.name,
        caloriesBurned: jsonResult.caloriesBurned,
        durationMinutes: durationMinutes,
        type: jsonResult.type as ExerciseType,
        notes: jsonResult.notes
    };
};
